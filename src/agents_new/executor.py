...
def execute_agent(
    ...
    agent_function_mapping = {
        AgentName.ROADMAP: "RoadmapAgent"
    }
    ...
    try:
        ...
        if agent_name not in agent_function_mapping:
            ...
            raise ValueError(f"Unsupported agent: {agent_name}. Available: {[a.value for a in available_agents]}")
        ...
        if agent_name == AgentName.ROADMAP:
        ...
        logger.info(f"Executing {agent_name.value} agent using BAMLService")
