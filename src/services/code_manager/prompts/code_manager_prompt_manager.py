...
class CodeManagerPromptManager:
    ...
    def get_system_prompt(self) -> str:
        ...
        prompt_parts = [
            ...
            "",
            "Your role is to:",
            "1. Analyze tool results in <thinking> tags",
            "2. Identify connection code between user's own services",
            "3. Return XML format with extracted connection code",
            "4. Return nothing if no connection code is found",
            "",
            "Focus on actual connection identifiers like API endpoints, queue names, and socket events that establish communication between different user services.",
            "",
            self.objective,
            "",
            self.capabilities,
    ...
    def get_user_prompt(self, tool_results: str) -> str:
        ...
        prompt_parts = [
            ...
            "",
            "Remember:",
            "- Use <thinking> tags to analyze the tool results",
            "- Extract connection code between user's own services only",
            "- Return <connection_code> XML format if connection code is found",
            "- Return nothing if no connection code is found",
            "- Focus on actual connection identifiers (endpoints, queue names, event names)",
        ]
