...
class ConnectionMatchingService:
    ...
    def match_connections(self, incoming_connections: List[Dict] = None, outgoing_connections: List[Dict] = None,
        ...
        try:
            ...
            if incoming_connections is None:
                incoming_connections = self.matching_manager.fetch_incoming_connections_from_db(project_id)
            if outgoing_connections is None:
                outgoing_connections = self.matching_manager.fetch_outgoing_connections_from_db(project_id)
            
            logger.info(f"Starting connection matching for {len(incoming_connections)} incoming and {len(outgoing_connections)} outgoing connections")
            ...
            for attempt in range(max_retries):
                try:
                    logger.info(f"Connection matching attempt {attempt + 1}/{max_retries}")
                    
                    # Get LLM response
                    llm_response = llm_client.call_llm("", matching_prompt, return_raw=True)
                    ...
                    if matching_results:
                        ...
                        logger.warning(f"{last_error}. Raw response: {llm_response[:200]}...")
                        
                        if attempt < max_retries - 1:
                            logger.info(f"Retrying connection matching (attempt {attempt + 2}/{max_retries})")
                        
                except Exception as e:
                    last_error = f"Error during LLM analysis on attempt {attempt + 1}: {str(e)}"
                    logger.error(last_error)
                    
                    if attempt < max_retries - 1:
                        logger.info(f"Retrying connection matching due to error (attempt {attempt + 2}/{max_retries})")
            ...
            is_valid, processed_results = self.matching_manager.validate_and_process_matching_results(
            ...
            return {
                ...
                "statistics": {
                    ...
                    "match_rate": round((processed_results.get("total_matches", 0) / 
                                      max(len(incoming_connections) + len(outgoing_connections), 1)) * 100, 2)
                }
            }
    ...
    def _parse_and_validate_response(self, llm_response: str) -> Optional[Dict]:
        ...
        try:
            ...
            for i, match in enumerate(parsed_json['matches']):
                required_match_fields = ['outgoing_id', 'incoming_id', 'match_confidence', 'match_reason', 'connection_type']
                for field in required_match_fields:
                    if field not in match:
                        logger.error(f"Missing required field '{field}' in match {i}")
                        return None
            
            logger.debug(f"Successfully parsed and validated JSON response with {len(parsed_json['matches'])} matches")
            return parsed_json
    ...
    def _store_matching_results(self, matching_results: Dict, project_id: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            summary_data = {
                ...
                "unmatched_incoming_count": len(matching_results.get("unmatched_incoming", [])),
                "unmatched_outgoing_count": len(matching_results.get("unmatched_outgoing", [])),
            ...
            logger.info(f"Stored {len(stored_matches)} match relationships (no duplicate connection data)")
    ...
    def _insert_connection_match(self, match_data: Dict) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"Inserted match relationship: outgoing_id={outgoing_id} -> incoming_id={incoming_id}")
    ...
    def _insert_matching_summary(self, summary_data: Dict) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Connection matching summary: {json.dumps(summary_data, indent=2)}")
    ...
    def get_matching_history(self, project_id: str = None, limit: int = 10) -> List[Dict]:
        """
    ...
    def get_connection_matches(self, project_id: str = None, confidence_filter: float = None) -> List[Dict]:
        """
