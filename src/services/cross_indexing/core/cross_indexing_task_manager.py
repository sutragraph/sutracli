...
class CrossIndexingTaskManager(SutraMemoryManager):
    ...
    def __init__(self):
        ...
        # Initialize CrossIndexing for task filtering
    ...
    def set_current_phase(self, phase: int) -> None:
        ...
        if 1 <= phase <= 5:
            ...
            if (
                ...
                and previous_phase in [1, 2]
                and phase == previous_phase + 1
            ):

                from loguru import logger

                logger.info(
                    f"Transitioning from Phase {previous_phase} to Phase {phase} - triggering task filtering"
                )
            ...
            # BUT preserve memory between Phase 3→4 and Phase 4→5 since they need shared context
            if hasattr(self, "current_phase") and self.current_phase != phase:
                # Don't clear memory when transitioning from Phase 3 to 4 or Phase 4 to 5
                if not (
                    (self.current_phase == 3 and phase == 4)
                    or (self.current_phase == 4 and phase == 5)
                ):
                    from loguru import logger

                    logger.info(
                        f"Clearing memory for phase transition: {self.current_phase} → {phase}"
                    )
                    ...
                    logger.info(
                        f"Preserving memory for phase transition: {self.current_phase} → {phase} (code snippets and history maintained)"
                    )
    ...
    def get_current_phase_tasks(self) -> Dict[str, List[Task]]:
        ...
        filtered_tasks = {"pending": [], "current": [], "completed": []}
        ...
        for category, tasks in all_tasks.items():
            for task in tasks:
                ...
                created_in_phase = task_metadata.get(
                    "created_in_phase", self.current_phase
                )
    ...
    def filter_tasks_for_next_phase(self, completed_phase: int) -> bool:
        ...
        logger.info(
            f"Starting task filtering after Phase {completed_phase} for Phase {next_phase}"
        )
        ...
        try:
            ...
            logger.info(
                f"Found {len(tasks_to_filter)} tasks to filter from Phase {completed_phase}"
            )
            ...
            logger.info(
                f"Successfully filtered tasks: {len(tasks_to_filter)} -> {len(filtered_tasks)} tasks"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error during task filtering for Phase {completed_phase}: {e}"
            )
            return False
