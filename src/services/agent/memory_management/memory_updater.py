...
class MemoryUpdater:
            ...
            try:
                ...
                if update_info:
                    if update_info.should_remove:
                        ...
                        logger.debug(f"Removed code snippet {code_snippet.id} - content no longer exists")
                    else:
                        # Update the code snippet
                        success = self._apply_code_snippet_update(code_snippet, update_info)
                        if success:
    ...
    def detect_line_changes(self, old_content: str, new_content: str) -> List[LineChange]:
        """
        ...
        while old_idx < len(old_lines) and new_idx < len(new_lines):
            if old_lines[old_idx] == new_lines[new_idx]:
                ...
                if old_idx + 1 < len(old_lines) and old_lines[old_idx + 1] == new_lines[new_idx]:
                    ...
                    old_idx += 1
                elif new_idx + 1 < len(new_lines) and old_lines[old_idx] == new_lines[new_idx + 1]:
