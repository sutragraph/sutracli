...
class ClaudeClient(LLMClientBase):
    ...
    def call_llm(self, system_prompt: str, user_message: str) -> List[Dict[str, Any]]:
        """Call Claude with separate system prompt and user message."""
        max_retries = 3
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.debug(
                    f"📦 Sending request to Claude model {self.model_id} with body: {body}"
                )
                ...
                logger.debug(f"📥 Received response from Claude model: {raw_response}")
