...
class PythonExtractor(BaseExtractor):
    ...
    def _get_keywords(self) -> Set[str]:
        ...
        stdlib = {
            # os module
            'path', 'environ', 'getcwd', 'chdir', 'mkdir', 'makedirs', 'remove',
            'removedirs', 'rename', 'renames', 'replace', 'rmdir', 'unlink',
            # sys module
            'argv', 'exit', 'modules', 'path', 'platform', 'stderr', 'stdin',
            ...
            # contextlib
            'contextmanager', 'asynccontextmanager', 'suppress', 'closing',
            # functools
            'partial', 'partialmethod', 'reduce', 'lru_cache', 'singledispatch',
            # itertools
            ...
            # asyncio
            'create_task', 'gather', 'get_event_loop', 'run', 'sleep', 'wait',
            'wait_for', 'AbstractEventLoop', 'Future', 'Task',
            # json
            'loads', 'dumps', 'load', 'dump', 'JSONEncoder', 'JSONDecoder',
            # re
            'match', 'fullmatch', 'search', 'sub', 'subn', 'split', 'findall',
            'finditer', 'compile', 'Pattern', 'Match'
        }
    ...
    def _extract_recursive(self, node: Any, symbols: List[Symbol], extract_calls: bool = False, defined_names: Set[str] = None):
        ...
        if extract_calls and defined_names is not None:
            ...
            if node_type in ['function_definition', 'async_function_definition']:
                ...
                name = self._find_identifier(node)
                if name and self._is_valid_symbol(name):
                    symbols.append(self._create_symbol(node, name, SymbolType.DECORATOR))
