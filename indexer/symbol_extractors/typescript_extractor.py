...
class TypeScriptExtractor(BaseExtractor):
    ...
    def _extract_recursive(
        ...
        if extract_calls and defined_names is not None:
            ...
            if node_type in ["function_declaration", "function", "method_definition"]:
                ...
                if name and self._is_valid_symbol(name):
                    symbols.append(
                        self._create_symbol(node, name, SymbolType.INTERFACE)
                    )
                ...
                for name in names:
                    if self._is_valid_symbol(name):
                        symbol_type = (
                            SymbolType.CONSTANT
                            if self._is_const_declaration(node)
                ...
                for name in names:
                    if self._is_valid_symbol(name):
                        symbols.append(
                            self._create_symbol(node, name, SymbolType.ENUM_MEMBER)
                        )
                ...
                name = (
                    self._get_text(node)
                    if node_type == "property_identifier"
                    else self._find_identifier(node)
                )
                if name and self._is_valid_symbol(name):
                ...
                if hasattr(child, "type") and child.type == "property_identifier":
                    ...
                    for grandchild in child.children:
                        if (
                            ...
                            and grandchild.type == "property_identifier"
                        ):
            ...
            for child in node.children:
                if hasattr(child, "type"):
                    if child.type == "identifier":
                        ...
                        obj_name = self._get_member_object_name(child)
    ...
    def _get_member_object_name(self, node: Any) -> str:
        """Extract the object name from a member expression."""
        ...
        if hasattr(first_child, "type"):
            if first_child.type == "identifier":
                ...
                # For nested member expressions like a.b.c, return the immediate object (b)
